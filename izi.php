...
const IZI_UPLOAD_DIR_URL = "https://aavi.xyz/wip/izi/files/";

//Impose file size limit?
const IZI_FILE_SIZE_LIMIT = true;
const IZI_MAX_FILE_SIZE = 1024*1000; //1024*x = x kilobytes
...
const IZI_INVALID_FORMATS = array("php", "exe", "cmd", "vbs", "bat");

//Enforce HTTPS?
const IZI_FORCE_HTTPS = false;
...
function printFooter(){
	if(isLoggedIn()){
		echo<<<ENDEND
<hr>
<small><a href="?m=logout">Logout</a> | <a href="?m=uploadFiles">Upload Files</a> | <a href="?m=listFiles">My Files</a> | <a href="?m=deleteFiles">Delete Files</a> | <a href="?m=config">View Config</a> | <a href="https://github.com/aaviator42/izi">Help/Source</a></small>
</body>
</html>
...
ENDEND;
	...
	echo <<<ENDEND
	  	<input type="file" id="file" name="files[]" multiple="multiple" accept="*" onchange="javascript:updateList()" />
...
function deleteFiles(){
	...
	if(isset($_POST["delfiles"])){
		...
		if(trim($_POST["delfiles"]) === "YOLO_DELETE_ALL"){
			...
			echo "<tr><td colspan='4'>All files have been deleted.</td></tr>";
...
function printFileList(){
	...
	if(isset($_GET["del"])){
		...
		if(file_exists(IZI_UPLOAD_DIR . $filename)){
			unlink(IZI_UPLOAD_DIR . $filename);
			echo "<tr><td colspan='4'><b>" . $filename . "</b> has been deleted. </td></tr>" . PHP_EOL;
		}
	}
	...
	foreach($files as $f){
		...
		echo "<tr><td><a href=" . $URL . ">" . $f . "</a></td><td>" . date("Y-m-d H:i:s", filemtime(IZI_UPLOAD_DIR . $f)) . "</td><td><a href='?m=listFiles&del=" . $f . "'>delete</a></td><td><a href='#' onclick='copyText(\"" . $URL . "\")'>copy url</a></td></tr>" . PHP_EOL;
	}
	if(empty($files)){
		echo "<tr><td colspan='4'>No files to show.</td></tr>" . PHP_EOL;
	}
	echo"<tr><td colspan='4'>Current date: " . date("Y-m-d H:i:s", time()) . "</td></tr>";
	echo "</table>";
}
